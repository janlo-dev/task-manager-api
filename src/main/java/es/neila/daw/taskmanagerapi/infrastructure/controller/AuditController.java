...
public class AuditController {
    ...
    public List<AuditLog> getByEntity(@PathVariable UUID entityId, Authentication authentication) {
        UUID currentUserId = UUID.fromString(authentication.getName());
        return getAuditLogByEntityUseCase.execute(entityId, currentUserId);
    }

    @GetMapping("/user/{userId}")
    public List<AuditLog> getByUser(@PathVariable UUID userId, Authentication authentication) {
        UUID currentUserId = UUID.fromString(authentication.getName());
        return getAuditLogByUserUseCase.execute(userId, currentUserId);
    }
}
